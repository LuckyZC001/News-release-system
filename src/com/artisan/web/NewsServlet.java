...
public class NewsServlet extends HttpServlet{
	...
	private String genUpAndDownPageCode(List<News> upAndDownPage){
		...
		if(upNews.getNewsId()==-1){
			pageCode.append("<p>back£ºno more</p>");
		}else{
			pageCode.append("<p>back£º<a href='news?action=show&newsId="+upNews.getNewsId()+"'>"+upNews.getTitle()+"</a></p>");
		}
		if(downNews.getNewsId()==-1){
			pageCode.append("<p>next£ºno more</p>");
		}else{
			pageCode.append("<p>next£º<a href='news?action=show&newsId="+downNews.getNewsId()+"'>"+downNews.getTitle()+"</a></p>");
		}
	...
	private void newsPreSave(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			if(StringUtil.isNotEmpty(newsId)){
				request.setAttribute("navCode", NavUtil.genNewsManageNavigation("News management", "News to modify"));				
			}else{
				request.setAttribute("navCode", NavUtil.genNewsManageNavigation("News management", "News to added"));				
			}
	...
	private void newsBackList(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			request.setAttribute("navCode", NavUtil.genNewsManageNavigation("News management", "News list"));
